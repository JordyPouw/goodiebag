...
const { abi: WETH } = require('@uniswap/v3-periphery/artifacts/contracts/interfaces/external/IWETH9.sol/IWETH9.json');

const ETH = ethers.BigNumber.from((1e18).toString());

async function getWMatic() {
	return await ethers.getContractAt('IERC20', '0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270');
}
...
async function printTokenBalance(goodieBag, nft) {
	...
	for (let i = 0; i < count; i++) {
		...
		console.log('Token', nft.toString(), 'balance', tokenAddress, tokenBalance / ETH.toString());
	}
}
...
describe('GoodieBag', function() {
	...
	it('Mint goodiebag with 5 matic', async () => {
		...
		const wmaticOnToken = await goodieBag.getNFTTokenBalance('0', wmatic.address);
		expect(wmaticOnToken).to.be.equal(ETH.mul(5));
		console.log(wmaticOnToken, '  the coins after mins');
	});
	...
	it('Should prevent non owners to redeem', async () => {
		const wmatic = await getWMatic();
		await goodieBag.connect(owner).mint({ value: ETH.mul(20) });
		expect(goodieBag.connect(account1).redeem('0')).to.be.revertedWith('Not allowed');
	});
	it('Transfer goodiebag', async () => {
		...
		await goodieBag.connect(owner).transferFrom(owner.address, account1.address, '0');
